      ...
      >
        ...
        <div className='p-5 border-b border-gray-100 flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            <h3 className='font-black text-gray-950 text-lg'>Your Shopping Cart</h3>
          </div>
          <button
            onClick={() => setIsCartOpen(false)}
        ...
        <div className='flex-grow overflow-y-auto p-5 space-y-4'>
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className='flex gap-4 p-3 border border-gray-100 rounded-2xl bg-white relative group shadow-xs'>
                ...
                <div className='flex flex-col justify-between py-0.5 flex-grow pr-6'>
                  <div>
                    ...
                    <span className='text-[10px] text-gray-400 font-bold capitalize'>{item.category}</span>
                  </div>
                  
                  <div className='flex items-center justify-between mt-2'>
                    <span className='text-sm font-extrabold text-gray-950'>${item.current_price}</span>
        ...
        {cart.length > 0 && (
          <div className='p-5 border-t border-gray-100 bg-gray-50/50 rounded-t-3xl'>
            <div className='flex items-center justify-between mb-4'>
              <span className='text-sm font-semibold text-gray-500'>Order Total Amount:</span>
              <span className='text-xl font-black text-gray-950'>${cartTotal}</span>
            </div>
            <button className='w-full bg-[#127058] hover:bg-[#0e5845] text-white font-bold py-3.5 px-4 rounded-xl transition-colors text-sm shadow-sm flex items-center justify-center gap-2'>
